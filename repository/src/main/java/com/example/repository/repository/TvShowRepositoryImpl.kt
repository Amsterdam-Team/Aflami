...
import com.example.repository.mapper.remote.toLocalTvShows
import com.example.repository.mapper.remote.toTvShows
...
class TvShowRepositoryImpl(
    ...
    private suspend fun getTvShowFromLocal(keyword: String): List<TvShow> {
        return tryToExecute(
            ...
            onSuccess = { localTvShows -> localTvShows.map { it.toDomain() } },
            onFailure = { emptyList() },
        )
    }

    private suspend fun getTvShowsFromRemote(keyword: String): List<TvShow> {
        return tryToExecute(
    ...
    private suspend fun saveTvShowsToDatabase(
        ...
        val localTvShows = remoteTvShows.toLocalTvShows()
        tryToExecute(
            function = {
